...
func (rs *RoleService) AddRole(roleName string) (*systemModel.RoleModel, error) {
	...
	return &roleModel, global.DB.Create(&roleModel).Error
...
func (rs *RoleService) DeleteRole(id uint) (err error) {
	...
	err = global.DB.Where("id = ?", id).First(&roleModel).Error
	if err != nil {
		return fmt.Errorf("查询role -> %v", err)
	}

	if !errors.Is(global.DB.Where("role_model_id = ?", id).First(&systemModel.UserModel{}).Error, gorm.ErrRecordNotFound) {
...
func (rs *RoleService) EditRole(eRole systemReq.EditRole) (err error) {
	var roleModel systemModel.RoleModel
	err = global.DB.Where("id = ?", eRole.ID).First(&roleModel).Error
	if err != nil {
		global.LOG.Error("查询角色", zap.Error(err))
	}
...
func (rs *RoleService) EditRoleMenu(roleId uint, ids []uint) (err error) {
	...
	var roleModel systemModel.RoleModel
	err = global.DB.Where("id = ?", roleId).First(&roleModel).Error
	if err != nil {
		global.LOG.Error("EditRoleMenu 查询role", zap.Error(err))
		return err
	}
