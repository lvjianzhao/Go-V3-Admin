...
func checkAndUpdateServiceStatus() {
	global.LOG.Info("定时任务 【xxx】 开始执行...")
	// 执行定时任务的代码
	global.LOG.Info("定时任务 【xxx】 执行完成...")
}
...
func addCron(spec string, cmd func()) {
	...
	id, err := global.CRON.AddFunc(spec, cmd)
	if err != nil {
		global.LOG.Info(
			fmt.Sprintf("Add Cron Failure,spec: %s,cron: %s,error: %s", spec, funcName, err))
	} else {
		global.LOG.Info(fmt.Sprintf("Add Crontab Success; ID: %v,task: %s", id, funcName))
	}
}
